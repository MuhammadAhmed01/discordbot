...
// Helper function for selecting random tile
...
// Functionality for clicking buttons
client.on('interactionCreate', async interaction => {
    ...
    if (isGameOver()) {
        ...
        interaction.update({
            content: "You won the game of tic-tac-toe " + (user.get('score') + 1) + " time(s)!",
            components: []
        })
        return;
    }
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'tictactoe') {
        ...
        await interaction.reply({ content: 'Play a game of tic-tac-toe', components: makeGrid() });
    }
})
